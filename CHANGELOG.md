...
- Added the first real `/visual-stage` Creation Board object-model slice: a WYSIWYG 创作案板 shell with typed creative objects, lineage fixtures, visible text/image/brand/artboard/deliverable nodes, object selection context, desktop canvas-first layout, and mobile Focus Lens/Object Stack/Bottom Inspector/Relationship Peek without calling provider APIs.
...
- Fixed the Visual Stage functional-review blockers: task event streams now return a real SSE observable synchronously, emit named `task.snapshot` events that browser listeners actually receive, and the client starts a polling watchdog if an SSE connection is silent, so already-succeeded generated drafts reconcile back into the conversation; the real React Flow canvas is also visible on mobile instead of being hidden behind the desktop breakpoint.
- Fixed the Visual Stage review blocker: `/visual-stage` now uses a real React Flow infinite canvas with zoom/pan/minimap/visible lineage edges, removes static-shell copy, renders committed session images as actual board image objects, and emits task stream updates when image generation reaches running/succeeded/failed so generated drafts appear in the conversation instead of waiting forever.
