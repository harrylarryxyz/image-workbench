...
- Fixed the Visual Stage functional-review blockers: task event streams now return a real SSE observable synchronously and the client starts a polling watchdog if an SSE connection is silent, so already-succeeded generated drafts reconcile back into the conversation; the real React Flow canvas is also visible on mobile instead of being hidden behind the desktop breakpoint.
