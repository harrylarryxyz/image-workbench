...
- Composer draft text must never be mirrored into the chat thread before send; sending clears the composer, generated draft cards are independent of the current 出图 toggle, polling should immediately reconcile terminal task state when SSE is unavailable, refresh should auto-scroll the thread to the latest exchange, adding reference images must not create automatic chat descriptions, and reference trays show visible compact `@图片N` chips with deletion, reference role tags, and horizontal scrolling instead of preview thumbnails or full filenames.
- The creation assistant may use a server-side OpenAI-compatible chat model, but must fall back to local Chinese guidance when the model is unavailable; UI and responses must not expose provider routes, API keys, storage keys, raw JSON, or debug payloads.
- Generated result UX is comparison-first: multi-image task results render as a comparison set, the user explicitly chooses a champion, “继续改” seeds the composer from the champion, and only the selected champion can be committed into the lightweight canvas preview.
- Creation Board MVP is not a task log: it shows the confirmed creative intent, reference-role chips, selected main image, branch count, and a reuse action that turns the main image back into a `@图片N` style/history reference for the next iteration.
