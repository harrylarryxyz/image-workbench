...
export default async function GalleryPage() {
  ...
  return <section>
    ...
    <div className="gallery" style={{marginTop: 20}}>{images.map((image, i) => <div className="card" key={image.id ?? i}>
      {image.storageKey ? <img className="thumb-img" src={`${API_BASE}/assets/${image.storageKey}`} alt={image.prompt ?? 'generated image'} /> : <div className="thumb">{image.format ?? 'image'}</div>}
