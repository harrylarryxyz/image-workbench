...
export type ModelCapability = {
  ...
  maxRefs?: number;
  /** Deprecated aggregate hint for older clients. Prefer task-type-specific limits below. */
  maxOutputCount?: number;
  maxGenerateOutputCount?: number;
  maxEditOutputCount?: number;
  /** Provider-call batch limit for /images/edits. Internal tasks may split a larger count into multiple calls. */
  maxProviderEditOutputCount?: number;
  recommendedTimeoutSec?: number;
  sizes?: string[];
  qualities?: string[];
  formats?: string[];
  apiModes?: ApiMode[];
...
const BUILTIN_MODEL_CAPABILITIES: Record<string, ModelCapability> = {
  'gpt-image-2': { model: 'gpt-image-2', supportsGenerate: true, supportsEdit: true, supportsMask: true, supportsTransparent: false, supportsMultipleRefs: true, maxRefs: 4, maxOutputCount: 4, maxGenerateOutputCount: 4, maxEditOutputCount: 4, maxProviderEditOutputCount: 1, recommendedTimeoutSec: 600, sizes: DEFAULT_SIZES, qualities: DEFAULT_QUALITIES, formats: DEFAULT_FORMATS, apiModes: ['auto', 'images', 'responses'] },
  'gpt-image-1.5': { model: 'gpt-image-1.5', supportsGenerate: true, supportsEdit: true, supportsMask: true, supportsTransparent: true, supportsMultipleRefs: true, maxRefs: 4, maxOutputCount: 4, maxGenerateOutputCount: 4, maxEditOutputCount: 4, maxProviderEditOutputCount: 4, recommendedTimeoutSec: 300, sizes: DEFAULT_SIZES, qualities: DEFAULT_QUALITIES, formats: DEFAULT_FORMATS, apiModes: ['auto', 'images'] },
};
