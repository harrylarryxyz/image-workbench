...
export class DiagnosticsService {
  classify(error: string | null | undefined): { code: string; suggestion: string } | null {
    ...
    if (msg.includes('502') || msg.includes('upstream request failed')) return { code: 'upstream_error', suggestion: 'The provider accepted the request path but its upstream image backend failed. Try another provider, reduce edit complexity, or test whether this provider supports /images/edits.' };
