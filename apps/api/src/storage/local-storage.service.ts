...
type StoredImage = { storageKey: string; format: string; sizeBytes: number; sha256: string; backend: StorageBackend; assetUrl?: string };
...
export class LocalStorageService {
  ...
  async putImage(bytes: Uint8Array): Promise<StoredImage> {
    ...
    return { storageKey, format: ext, sizeBytes: bytes.byteLength, sha256, backend: this.backend, assetUrl: this.publicUrl(storageKey) };
  }
