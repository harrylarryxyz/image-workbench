...
export async function serializeProvider(prisma: PrismaService, row: any, includeHealth = false) {
  ...
  return {
    ...
    capabilities: {
      ...
      maxGenerateOutputCount: capability?.maxGenerateOutputCount ?? null,
      maxEditOutputCount: capability?.maxEditOutputCount ?? null,
      maxProviderEditOutputCount: capability?.maxProviderEditOutputCount ?? null,
